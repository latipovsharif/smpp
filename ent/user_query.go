...
func (uq *UserQuery) QueryUserMessages() *UserMonthMessageQuery {
	query := &UserMonthMessageQuery{config: uq.config}
...
func (uq *UserQuery) QueryMessages() *MessagesQuery {
	query := &MessagesQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withRateID:       uq.withRateID.Clone(),
...
func (uq *UserQuery) WithUserMessages(opts ...func(*UserMonthMessageQuery)) *UserQuery {
	query := &UserMonthMessageQuery{config: uq.config}
...
func (uq *UserQuery) WithMessages(opts ...func(*MessagesQuery)) *UserQuery {
	query := &MessagesQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withUserMessages; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.UserMessages = []*UserMonthMessage{}
		}
		query.withFKs = true
		query.Where(predicate.UserMonthMessage(func(s *sql.Selector) {
			s.Where(sql.InValues(user.UserMessagesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.UserMessages = append(node.Edges.UserMessages, n)
		}
	}
	...
	if query := uq.withRateID; query != nil {
